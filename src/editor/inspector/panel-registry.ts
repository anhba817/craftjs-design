...
export interface PanelDefinition {
  // Stable identifier. Matches `CanonicalComponent.applicablePanels` entries
  // when a canonical explicitly whitelists panels. Built-ins use:
  // 'layout' | 'size' | 'spacing' | 'typography' | 'appearance' | 'effects'
  // | 'componentProps'.
  id: string
  ...
  // `applicablePanels` whitelist. Return true to render the panel for this
  // canonical. If `applicablePanels` IS set on the canonical, this predicate
  // is ignored — only ids in the whitelist render.
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  applicableTo: (def: CanonicalComponent<any>) => boolean
  // The panel UI. Receives the selected node id and the active style slot
...
/**
 * Register an inspector panel. Re-registering an existing id replaces the
 * previous definition — used by SDK consumers overriding a built-in.
 */
